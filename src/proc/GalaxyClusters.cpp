...
static ClusterCenter clusterForCell(core::u64 universeSeed,
                                   core::i64 cx,
                                   core::i64 cy,
                                   core::i64 cz,
                                   const GalaxyClustersParams& p) {
  ClusterCenter c{};
  ...
  core::u64 h = core::hashCombine(universeSeed, core::fnv1a64("galaxy_cluster_cell_v1"));
  ...
  // Enable cluster with per-cell probability.
  ...
  c.id = core::hashCombine(core::hashCombine(universeSeed, core::fnv1a64("galaxy_cluster_id_v1")), h);
  return c;
}
...
static double falloff(double dist, double radius, double power) {
  ...
  // Exponent controls how tight the cluster is.
  ...
  for (core::i64 dz = -search; dz <= search; ++dz) {
    for (core::i64 dy = -search; dy <= search; ++dy) {
      for (core::i64 dx = -search; dx <= search; ++dx) {
        ClusterCenter c = clusterForCell(universeSeed, cx + dx, cy + dy, cz + dz, params);
