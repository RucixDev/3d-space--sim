...
// -----------------------------------------------------------------------------
// Crater field synthesis
//
// These helpers generate seam-free crater impressions directly on the unit sphere.
// They are intentionally “procedural only” (no precomputed crater lists) so the
// CPU path stays deterministic and the GPU path can mirror the same idea.
//
// The core primitive is a tiny 3D Worley-style nearest-feature lookup over a
// jittered integer grid in (x,y,z) space, evaluated on p*freq where p is the
// unit-sphere direction. We then shape that distance into a bowl + rim + ejecta
// ring, with optional directional "rays" in the ejecta.

struct CraterSample {
  double height{0.0}; // height delta to add to the base height field
  double bowl{0.0};   // 0..1 interior bowl mask
  double rim{0.0};    // 0..1 rim ring mask
  double ejecta{0.0}; // 0..1 exterior ejecta mask (may include rays)
};
...
static inline Vec3 addv(const Vec3& a, const Vec3& b) { return {a.x + b.x, a.y + b.y, a.z + b.z}; }

static inline Vec3 mulv(const Vec3& a, double k) { return {a.x * k, a.y * k, a.z * k}; }
...
static inline Vec3 normalizeSafe(const Vec3& v, const Vec3& fallback) {
  const double len2 = dot(v, v);
  if (len2 <= 1e-18) return fallback;
  const double inv = 1.0 / std::sqrt(len2);
  return {v.x * inv, v.y * inv, v.z * inv};
}

static inline double hash01FromU64(core::u64 h) {
  // Convert to a [0,1) double using 53 bits (mantissa precision).
  return (double)(h >> 11) * (1.0 / 9007199254740992.0); // 2^53
}

static inline core::u64 cellHash(core::u64 seed, int x, int y, int z) {
  core::u64 h = seed;
  h = core::hashCombine(h, (core::u64)(core::u32)x);
  h = core::hashCombine(h, (core::u64)(core::u32)y);
  h = core::hashCombine(h, (core::u64)(core::u32)z);
  return h;
}
...
struct WorleyHit {
  double dist{1.0e9};
  core::u64 id{0};
  Vec3 feature{0.0, 0.0, 0.0};
};

static WorleyHit worleyNearest(core::u64 seed, double x, double y, double z) {
  ...
  for (int dz = -1; dz <= 1; ++dz) {
    for (int dy = -1; dy <= 1; ++dy) {
      for (int dx = -1; dx <= 1; ++dx) {
        ...
        const core::u64 id = cellHash(seed, cx, cy, cz);

        const double rx = hash01FromU64(core::hashCombine(id, 0xA531u));
        const double ry = hash01FromU64(core::hashCombine(id, 0xC0DEu));
        const double rz = hash01FromU64(core::hashCombine(id, 0xBEEFu));

        const Vec3 f{(double)cx + rx, (double)cy + ry, (double)cz + rz};

        const double dxp = x - f.x;
        const double dyp = y - f.y;
        const double dzp = z - f.z;
        const double d = std::sqrt(dxp * dxp + dyp * dyp + dzp * dzp);
...
static CraterSample craterLayer(core::u64 seed, const Vec3& p, double freq, double amp, bool withRays) {
  const WorleyHit hit = worleyNearest(seed, p.x * freq, p.y * freq, p.z * freq);

  // Per-crater shaping parameters.
  const double radius = lerp(0.28, 0.58, hash01FromU64(core::hashCombine(hit.id, 0xC0DEC0DEull)));
  const double depthScale = lerp(0.75, 1.40, hash01FromU64(core::hashCombine(hit.id, 0xBEEF1234ull)));

  const double d = hit.dist / std::max(1.0e-6, radius);

  // Bowl: smooth depression toward the center.
  const double t = clamp01(1.0 - d);
  const double bowl = t * t;

  // Rim: a narrow raised ring around d~=1.
  const double rim = smoothstep(0.86, 1.0, d) * (1.0 - smoothstep(1.0, 1.14, d));

  // Ejecta ring: wider influence outside the rim.
  const double ejectaRing = smoothstep(1.0, 1.08, d) * (1.0 - smoothstep(1.08, 1.70, d));
  double ejecta = ejectaRing;

  if (withRays && ejectaRing > 1.0e-5) {
    // Approximate crater center direction by projecting the feature point back onto the unit sphere.
    const Vec3 cpos{hit.feature.x / freq, hit.feature.y / freq, hit.feature.z / freq};
    const Vec3 cdir = normalizeSafe(cpos, p);

    Vec3 up{0.0, 1.0, 0.0};
    if (std::abs(cdir.y) > 0.85) up = {1.0, 0.0, 0.0};

    const Vec3 t0 = normalizeSafe(cross(up, cdir), Vec3{1.0, 0.0, 0.0});
    const Vec3 t1 = cross(cdir, t0);

    Vec3 v = sub(p, mulv(cdir, dot(p, cdir)));
    const double vLen2 = dot(v, v);
    if (vLen2 > 1.0e-12) {
      v = mulv(v, 1.0 / std::sqrt(vLen2));
      const double ang = std::atan2(dot(v, t1), dot(v, t0));

      const int rayCount = 6 + (int)(core::hashCombine(hit.id, 0x51DEu) % 15ull); // 6..20
      const double phase = hash01FromU64(core::hashCombine(hit.id, 0xA11CE5u)) * (2.0 * kPi);

      double rv = 0.5 + 0.5 * std::cos(ang * (double)rayCount + phase);
      rv = std::pow(clamp01(rv), 5.0); // sharpen
      ejecta *= lerp(0.35, 1.0, rv);
    }
  }

  CraterSample out{};
  out.height = (-amp * depthScale * bowl) + (amp * 0.45 * rim) + (amp * 0.10 * ejectaRing);
  out.bowl = bowl;
  out.rim = rim;
  out.ejecta = ejecta;
  return out;
...
static CraterSample craterField(core::u64 seed, const Vec3& p) {
  // Multi-scale mix: a few big impacts, then progressively smaller pock marks.
  const CraterSample a = craterLayer(seed ^ 0xC7A73ull, p, 2.2, 0.10, true);
  const CraterSample b = craterLayer(seed ^ 0xF00Dull,  p, 7.5, 0.05, false);
  const CraterSample c = craterLayer(seed ^ 0xB16D0ull, p, 16.0, 0.02, false);

  CraterSample out{};
  out.height = a.height + b.height + c.height;
  out.bowl = std::max(a.bowl, std::max(b.bowl * 0.75, c.bowl * 0.50));
  out.rim = std::max(a.rim, std::max(b.rim * 0.75, c.rim * 0.50));
  out.ejecta = std::max(a.ejecta, std::max(b.ejecta * 0.65, c.ejecta * 0.45));
  return out;
}
...
static Color surfaceRocky(core::u64 seed, const Vec3& p, double lat, double lon) {
  ...
  // Impact craters: bowl + rim + ejecta (with optional ray streaks).
  const CraterSample cr = craterField(seed ^ 0xC0DEF00Dull, p);
  col = lerp(col, mul(col, 0.55), cr.bowl * 0.25);
  col = lerp(col, Color{0.92, 0.90, 0.86}, cr.rim * 0.35);
  col = lerp(col, Color{0.88, 0.84, 0.78}, cr.ejecta * 0.18);
...
static Color surfaceDesert(core::u64 seed, const Vec3& p, double lat, double lon) {
  ...
  // Subtle crater weathering: sand fills bowls, bright dust on rims.
  const CraterSample cr = craterField(seed ^ 0xC0DEF00Dull, p);
  col = lerp(col, mul(col, 0.75), cr.bowl * 0.22);
  col = lerp(col, Color{0.97, 0.93, 0.78}, cr.rim * 0.28);
  col = lerp(col, Color{0.96, 0.92, 0.84}, cr.ejecta * 0.22);
...
static Color surfaceIce(core::u64 seed, const Vec3& p, double lat, double lon) {
  ...
  // Ice impacts tend to have softer relief and higher-albedo ejecta.
  const CraterSample cr0 = craterField(seed ^ 0xC0DEF00Dull, p);
  const double bowl = cr0.bowl * 0.65;
  const double rim = cr0.rim * 0.65;
  const double ej = cr0.ejecta * 0.55;
  col = lerp(col, Color{0.62, 0.74, 0.86}, bowl * 0.18);
  col = lerp(col, Color{0.98, 1.00, 1.00}, rim * 0.25);
  col = lerp(col, Color{0.90, 0.96, 1.00}, ej * 0.16);
...
static Color surfaceCityLights(core::u64 seed, const Vec3& p, double lat, double lon) {
  (void)lon;

  // Base continent field matched to SurfaceKind::Ocean so the emissive map aligns
  // with the same land masses when applied to ocean planets.
  const double cont = fbm3D(seed ^ 0x0CE4u, p.x * 2.1, p.y * 2.1, p.z * 2.1, 6, 2.0, 0.5);
  const double land = smoothstep(0.48, 0.55, cont);
  const double coast = clamp01(1.0 - std::abs(cont - 0.52) * 18.0);

  // Latitudinal preference: fewer lights near the poles.
  const double latW = std::pow(std::abs(std::cos(lat)), 0.30);

  // Population density field (multi-scale).
  const double pop0 = fbm3D(seed ^ 0xC1717u, p.x * 6.0, p.y * 6.0, p.z * 6.0, 5, 2.1, 0.55);
  const double pop1 = fbm3D(seed ^ 0xB16D0u, p.x * 14.0 + 12.3, p.y * 14.0 - 7.1, p.z * 14.0 + 3.7, 4, 2.2, 0.55);
  const double pop = clamp01(0.65 * pop0 + 0.35 * pop1);

  // City clusters.
  const double cluster = smoothstep(0.72, 0.92, pop);

  // Road / corridor filaments (ridged noise).
  const double r0 = fbm3D(seed ^ 0x70ADu, p.x * 22.0 + 4.1, p.y * 22.0 - 9.7, p.z * 22.0 + 2.6, 3, 2.3, 0.55);
  const double rid = 1.0 - std::abs(r0 * 2.0 - 1.0);
  double roads = smoothstep(0.78, 0.94, rid);
  roads *= (0.30 + 0.70 * cluster);

  // Mega-city hotspots: rare but bright.
  const double mega0 = fbm3D(seed ^ 0x31E6Au, p.x * 1.6 + 19.7, p.y * 1.6 - 5.2, p.z * 1.6 + 11.3, 4, 2.0, 0.5);
  double mega = smoothstep(0.78, 0.95, mega0);
  mega = mega * mega;

  // Combine into a density and carve holes so it doesn't blanket the land.
  double d = land * latW * (0.60 * cluster + 0.25 * roads + 0.55 * mega * (0.35 + 0.65 * coast));
  d *= (0.35 + 0.65 * coast);

  const double hole = fbm3D(seed ^ 0x4011Eu, p.x * 4.5 - 7.1, p.y * 4.5 + 2.2, p.z * 4.5 - 3.3, 4, 2.0, 0.5);
  d *= (1.0 - 0.55 * smoothstep(0.55, 0.80, hole));

  // Make the map sparse: drop low densities, sharpen the remaining ones.
  d = smoothstep(0.08, 0.28, d);
  d = std::pow(clamp01(d), 1.25);

  // Micro sparkle so large cities have some high-frequency detail.
  const double spark = fbm3D(seed ^ 0x5A2F01u, p.x * 55.0, p.y * 55.0, p.z * 55.0, 2, 2.5, 0.6);
  const double sparkMask = smoothstep(0.74, 0.93, spark);
  d = clamp01(d * (1.0 + 0.35 * sparkMask));

  // Warm/cool tint variation per seed (still generally "sodium-vapor" warm).
  const double warmT = clamp01(0.65 + 0.35 * ((double)((seed >> 16) & 0xFFull) / 255.0));
  Color cool{0.72, 0.88, 1.00};
  Color warm{1.00, 0.84, 0.58};
  Color tint = lerp(cool, warm, warmT);
...
static double surfaceHeight(SurfaceKind kind, core::u64 seed, const Vec3& p, double lat, double lon) {
  ...
  switch (kind) {
    ...
    case SurfaceKind::CityLights: {
      // Emissive-only map; keep the normal map essentially flat.
      return 0.5;
    }
    default:
      return 0.5;
    ...
    for (int x = 0; x < img.w; ++x) {
      ...
      switch (kind) {
        ...
        case SurfaceKind::Clouds:
          ...
          c = {0.94, 0.97, 1.00};
          break;
        case SurfaceKind::CityLights: c = surfaceCityLights(seed, p, lat, lon); break;
...
SurfaceImage generateSurfaceNormalMap(SurfaceKind kind, core::u64 seed, int widthPx) {
  ...
  switch (kind) {
    ...
    case SurfaceKind::CityLights: kindScale = 0.0; break;
