...
// Variant that assumes the target maintains a constant world-space acceleration
// during the projectile's flight (useful for agile targets and fire-control prediction).
//
// Intercept time solves:
//   |(targetPos - shooterPos) + (targetVel - shooterVel) * t + 0.5 * targetAccel * t^2| = projectileSpeed * t
// for t > 0.
//
// Returns the smallest positive intercept time (>= minTimeSec) if a valid solution exists,
// and requires tSec <= maxTimeSec.
std::optional<double> solveInterceptTimeSecAccel(
  const math::Vec3d& shooterPosKm,
  const math::Vec3d& shooterVelKmS,
  const math::Vec3d& targetPosKm,
  const math::Vec3d& targetVelKmS,
...
// Convenience wrapper that returns intercept time + lead point + unit aim direction,
// using the constant-acceleration target model above.
std::optional<LeadSolution> solveProjectileLeadAccel(
  const math::Vec3d& shooterPosKm,
  const math::Vec3d& shooterVelKmS,
  const math::Vec3d& targetPosKm,
  const math::Vec3d& targetVelKmS,
