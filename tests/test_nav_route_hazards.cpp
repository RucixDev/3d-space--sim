...
stellar::sim::SystemStub makeStub(stellar::sim::SystemId id, const stellar::math::Vec3d& posLy) {
  ...
  s.id = id;
  // SystemStub is deliberately lightweight (see include/stellar/sim/Celestial.h).
  // Populate only the fields that actually exist there.
  s.seed = static_cast<stellar::core::u64>(id);
  s.name = std::string("T") + std::to_string((unsigned long long)id);
  s.posLy = posLy;
